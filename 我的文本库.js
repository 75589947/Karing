    ...
    return `<div class="section">
      <div class="section-header" onclick="toggleSection('fileList')">
        ...
        <div class="header-status">
          <span class="selection-status" id="selectionInfo">未选择任何文件</span>
...
.selection-status,.editor-status{color:#718096;font-size:0.75em}
.toggle-icon{color:#718096;font-size:0.75em;transition:transform 0.3s}
.section-content{padding:0;max-height:0;overflow:hidden;transition:all 0.3s}
...
.file-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;width:100%}
.file-item{background:#f7fafc;padding:8px;border-radius:8px;border:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center}
...
.file-actions{gap:8px;flex-shrink:0;justify-content:center}
...
.file-checkbox{width:16px;height:16px;cursor:pointer;margin:0}
...
.editor{margin:0;width:100%;height:200px;border:2px solid #e2e8f0;padding:10px;font-size:13px;resize:vertical;outline:none;font-family:monospace;border-radius:8px;background:#fafbfc}
...
.share-input input{flex:1;padding:8px 10px;border:2px solid #e2e8f0;border-radius:6px;font-size:12px;min-width:120px;height:34px;box-sizing:border-box}
...
.btn{padding:8px 10px;border:2px solid;background:white;border-radius:6px;cursor:pointer;font-size:12px;display:flex;align-items:center;gap:3px;white-space:nowrap;height:34px;box-sizing:border-box}
...
@media (max-width:768px){
  ...
  .header-status{order:2;width:100%;justify-content:flex-end}
}
@media (max-width:480px){
  .file-grid{grid-template-columns:1fr}
  .file-management-controls .btn{font-size:9px;padding:3px 4px}
  .selection-status,.editor-status{font-size:0.65em}
}
</style>
</head>
<body>
<div class="container">
  ...
  <div class="section">
    <div class="section-header" onclick="toggleSection('notepad')">
      <h3>📝 在线记事本</h3>
      <div class="header-status">
        <span class="editor-status" id="status">就绪</span>
        <span class="toggle-icon">▼</span>
      </div>
    </div>
...
function toggleSection(sectionId) {
  ...
  const headerStatus = section.previousElementSibling.querySelector('.header-status');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const notepadHeaderStatus = notepadSection.previousElementSibling.querySelector('.header-status');
  
  notepadSection.classList.add('expanded');
  notepadIcon.style.transform = 'rotate(180deg)';
  // 记事本默认展开，所以显示状态信息
  notepadHeaderStatus.style.display = 'flex';
  
  const fileListSection = document.getElementById('fileList');
  if (fileListSection) {
    const fileListHeaderStatus = fileListSection.previousElementSibling.querySelector('.header-status');
    // 仓库列表默认折叠，所以隐藏状态信息
    fileListHeaderStatus.style.display = 'none';
  }
...
function editSelectedFile() {
  if (selectedFiles.size !== 1) return showRepoStatus('请选择且仅选择一个文件进行编辑');
  ...
  }).catch(() => showRepoStatus('获取文件内容失败'));
}

async function deleteSelectedFiles() {
  if (isDeleting) return;
  
  if (selectedFiles.size === 0) return showRepoStatus('请选择要删除的文件');
  ...
  try {
    const results = [];
    const filesToDelete = Array.from(selectedFiles.entries());
    
    for (let i = 0; i < filesToDelete.length; i++) {
      const [filename, sha] = filesToDelete[i];
      
      showRepoStatus('正在删除: ' + filename + ' (' + (i+1) + '/' + filesToDelete.length + ')');
      
      try {
        const response = await fetch('', { 
          method: 'POST', 
          body: 'github:' + JSON.stringify({ action: 'delete', filename, sha }) 
        });
    ...
    if (failCount === 0) {
      showRepoStatus('成功删除 ' + successCount + ' 个文件');
    } else {
      showRepoStatus('成功删除 ' + successCount + ' 个文件，失败 ' + failCount + ' 个');
    }
    ...
    showRepoStatus('删除过程中出错: ' + error.message);
  } finally {
    isDeleting = false;
    deleteBtn.disabled = false;
...
function handleFileAction() {
  ...
  fetch('', { method: 'POST', body: 'github:' + JSON.stringify(data) })
    ...
    .then(result => {
      if (result.error) showRepoStatus('操作失败: ' + result.error);
      else { 
        showRepoStatus(currentAction === 'create' ? '文件创建成功' : '文件更新成功'); 
        closeModal(); 
        reloadWithExpand(); 
      }
    }).catch(() => showRepoStatus('操作失败'));
}
